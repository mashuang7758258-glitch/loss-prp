    ...
    for loop_idx in range(loop_count):
        ...
        if "Stall" in inputs["op_mode"]:
            ...
            p_sw_diode_chip = (
                ...
                * (erec_adj / 1000.0)
                * math.sqrt(2.0)
                * (inputs["iout_rms"] / max(inputs["i_nom_ref"], 1e-12))
                * (inputs["vdc_act"] / max(inputs["v_ref"], 1e-12))
            )
...
with c3:
    ...
    kv_frd_default = 1.0 if "SiC" in device_type else 0.60
...
with st.expander("🔬 查看底层应用的完整物理公式与工程约定 (长期保留，不准删)", expanded=False):
    ...
    st.latex(r"P_{sw,frd} = \frac{1}{\pi}f_{sw}E_{rec,adj}\sqrt{2}\left(\frac{I_{out}}{I_{nom}}\right)\left(\frac{V_{dc}}{V_{nom}}\right)")
    st.caption("中文注释：Psw,frd 为二极管反向恢复损耗；fsw 为开关频率；Erec,adj 为修正后的单次恢复能量；Iout 为输出电流有效值；Inom 为规格书对标电流；Vdc/Vnom 为电压缩放。SiC 模式下二极管电压缩放指数 K_v_frd 默认设为 1.0。")
