...
def calc_switching_energy(
    ...
    2. 标称点比例法：先查 I_nom_ref 的标称能量，再按经验公式缩放；
       二极管默认按公司 Excel 口径执行 (K_i_frd * x + (1 - K_i_frd)) 的电流修正。
    """
    ...
    if "直线比例法" in algo_type:
        ...
        current_ratio = max(float(i_pk), 0.0) / nominal_current
        current_correction = (
            max(0.0, ki_frd * current_ratio + (1.0 - ki_frd))
            if is_diode
            else current_ratio
        )
...
def calc_pwm_conduction_losses(
    ...
    - 死区引起的导通路径重分配显式加入
    """
    main_v0 = 0.0 if is_sic else main_model["v0"]
    diode_v0 = diode_model["v0"]
    r_main_total = main_model["r_eq"] + r_pkg_chip + r_arm_chip
    r_diode_total = diode_model["r_eq"] + r_pkg_chip + r_arm_chip

    inst_main = main_v0 * i_pk_chip + r_main_total * i_pk_chip**2
    inst_diode = diode_v0 * i_pk_chip + r_diode_total * i_pk_chip**2
    ...
    if mode == "SVPWM" and is_sic:
        ...
        p_cond_diode = ((4.0 - m_eff * math.pi * active_cosphi) / 4.0) * (diode_v0 * i_pk_chip / math.pi) + diode_r_factor * (
        ...
        p_cond_main = (kv0_m * main_v0 * i_pk_chip) + (kr_m * r_main_total * i_pk_chip**2) / math.pi
        p_cond_diode = (kv0_d * diode_v0 * i_pk_chip) / math.pi + (kr_d * r_diode_total * i_pk_chip**2) / math.pi
    else:
        if is_sic:
            p_cond_main = r_main_total * i_pk_chip**2 * (1.0 / 8.0 + m_eff * active_cosphi / (3.0 * math.pi))
        else:
            p_cond_main = (
                main_v0 * i_pk_chip * (1.0 / (2.0 * math.pi) + m_eff * active_cosphi / 8.0)
                + r_main_total * i_pk_chip**2 * (1.0 / 8.0 + m_eff * active_cosphi / (3.0 * math.pi))
            )
        p_cond_diode = (
            diode_v0 * i_pk_chip * (1.0 / (2.0 * math.pi) - m_eff * active_cosphi / 8.0)
    ...
    if dead_ratio > 0.0:
        avg_inst_main = (main_v0 * i_pk_chip * 2.0 / math.pi) + (r_main_total * i_pk_chip**2 * 0.5)
        avg_inst_diode = (diode_v0 * i_pk_chip * 2.0 / math.pi) + (r_diode_total * i_pk_chip**2 * 0.5)
        if is_sic:
...
def calc_stall_losses(
    ...
    """
    is_sic = "SiC" in str(main_model.get("device_type", ""))
    main_v0 = 0.0 if is_sic else main_model["v0"]
    r_main_total = main_model["r_eq"] + r_pkg_chip + r_arm_chip
    r_diode_total = diode_model["r_eq"] + r_pkg_chip + r_arm_chip
...
def build_formula_audit_df(inputs: dict, dead_meta: dict, eon_meta: dict, eoff_meta: dict, erec_meta: dict) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            ...
            {"审计项": "开关能量提取算法", "当前实现": eon_meta["extraction_label"], "说明": "导通域与开关域电流独立评估；FRD 在线性标称点模式下执行 K_i_frd*x+(1-K_i_frd) 电流修正与独立 K_v_frd 电压修正"},
...
def simulate_system(inputs: dict, tables: dict):
    ...
    if "A. 公司 Excel 对标模式" in inputs.get("loss_eval_mode", "") and "SiC" in inputs["device_type"]:
        norm_ev_d = norm_ev_m.copy()
    ...
    for loop_idx in range(loop_count):
        ...
        main_model["device_type"] = inputs["device_type"]
        ...
        [
            {
                ...
                "R_tot_main (Ω)": cond_result["r_main_total"],
            ...
            },
            {
                ...
                "R_tot_diode (Ω)": cond_result["r_diode_total"],
    ...
    summary_df = pd.DataFrame(
        [
            ...
            {"指标": "损耗评估模式", "数值": inputs.get("loss_eval_mode", ""), "单位": ""},
    ...
    input_snapshot_df = pd.DataFrame(
        [
            ...
            {"参数": "损耗评估模式", "取值": inputs.get("loss_eval_mode", ""), "单位": ""},
...
with st.sidebar:
    ...
    loss_eval_mode = st.radio(
        "2. 损耗评估模式",
        ["A. 公司 Excel 对标模式 (全同步整流)", "B. 高精度物理模式 (考虑死区发热)"],
        help="模式 A 强制 t_dead = 0，并让 SiC 续流路径导通模型复制主开关；模式 B 保留真实死区与体二极管导通参数。",
    )
...
with col_diode:
    ...
    if "手动输入线性化参数" in cond_param_input_mode:
        st.caption("1. 手动线性化导通参数表 (V0 / R_dynamic)")
        st.caption("⚠️ 输入指引：仅在‘高精度物理模式’或‘IGBT 模式’下需输入；SiC 全同步整流对标时可不填。")
...
with c2:
    ...
    algo_type = st.radio(
        ...
        help="CAE 精确法直接按当前 Ipk 与 Tj 查二维矩阵；标称点比例法先取 I_nom 标称点，主开关按线性缩放，二极管按 K_i_frd*x+(1-K_i_frd) 做电流修正，并独立施加 K_v_frd 电压修正。",
    )
    if "直线比例法" in algo_type:
        i_nom_ref = st.number_input("直线基准电流 I_nom (A)", value=400.0, min_value=0.001)
    else:
        i_nom_ref = 400.0
    dead_time_default = 0.0 if "A. 公司 Excel 对标模式" in loss_eval_mode else 2.0
    dead_time_us = st.number_input(
        "死区时间 t_dead (us)",
        value=dead_time_default,
        min_value=0.0,
        format="%.3f",
        disabled="A. 公司 Excel 对标模式" in loss_eval_mode,
    )
    st.caption("提示：IGBT SPWM 模式或 SiC 全同步整流模式下，建议设为 0 以对齐效率报告；当研究 SiC 散热极限时，请输入实际死区（参考范围：0.5us ~ 3.0us）并补全二极管 V-I 表。")
...
with c3:
    ...
    kv_frd_default = 1.0
    kv_frd = st.number_input("二极管电压缩放指数 K_v_frd", value=kv_frd_default)
    ki_frd = st.number_input("续流电流经验系数 K_i_frd", value=0.45)
...
inputs = {
    ...
    "dead_time_us": 0.0 if "A. 公司 Excel 对标模式" in loss_eval_mode else float(dead_time_us),
    "kv_on": float(kv_on),
    "kv_off": float(kv_off),
    "kv_frd": float(kv_frd),
    "ki_frd": float(ki_frd),
...
if result:
    ...
    st.caption("以上特性参数对应当前计算工作点；结果表已额外显示 R_tot，且导通损耗统一使用 R_tot = R_dynamic + R_pkg + R_arm。")
...
with st.expander("🔬 查看底层应用的完整物理公式与工程约定 (长期保留，不准删)", expanded=False):
    ...
    st.caption("中文注释：P_cond,IGBT 为 IGBT 主开关导通损耗；M_eff 为考虑死区后的有效调制比；cosφ 为功率因数；VCE0 为主开关截距压降；Ipk 为单芯片峰值电流；Rtot 为动态电阻、封装电阻、桥臂附加电阻之和。")
    ...
    st.caption("中文注释：P_cond,D 为续流二极管导通损耗；VF0 为二极管截距压降；Rtot,D 为续流路径总等效电阻。")
    st.latex(r"P_{cond,SiC}^{SPWM} = \left(\frac{1}{8} + \frac{M_{eff}\cos\phi}{3\pi}\right)R_{tot,SiC}I_{pk}^2")
    st.caption("中文注释：SiC 主开关导通损耗在程序里强制只保留 Rtot,SiC·Ipk² 项，主芯片内部无条件锁定 V0 = 0。")
    ...
    st.caption("中文注释：Rdynamic 来自特性参数输入或 V-I 线性化；Rpkg,chip 是单芯片封装寄生电阻；Rarm,eq 是桥臂公共回路折算到单芯片路径的附加电阻。SVPWM 下 SiC 主芯片导通损耗使用的内阻，就是这里的 Rtot,SiC。")
    ...
    st.latex(r"E_{rec,base} = E_{rec}(I_{nom},T_j)\cdot \left(K_{i,frd}\frac{I_{pk}}{I_{nom}} + 1 - K_{i,frd}\right)")
    ...
    st.caption("中文注释：Psw,frd 为二极管反向恢复损耗；fsw 为开关频率；Erec,adj 为修正后的单次恢复能量。E_rec,adj 已在能量提取层级完成了电流、电压和温度的综合修正。")
    st.write("E-I 表无论是一维还是多温度矩阵，经验温漂系数都会显式参与修正；同时支持“CAE 精确二维插值”和“标称点直线比例法”两种能量提取路径。在线性标称点模式下，二极管按公司 Excel 口径执行 `(0.45x + 0.55)` 电流修正，且默认采用 1 次方电压修正。")
