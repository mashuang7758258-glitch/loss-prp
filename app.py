...
st.set_page_config(page_title="BYD全架构仿真-全景展现版", layout="wide")
st.title("🛡️ 功率模块多架构电热联合仿真平台 (全景对标版)")
...
# ==========================================
st.divider()
st.header("📊 第一步：特性数据录入 (支持 Excel 复制粘贴)")
...
with c1:
    ...
    fout = st.number_input("输出频率 f_out (Hz)", value=200.0, help="保留作为工况展示，不影响稳态平均功率")
...
if st.button("🚀 执 行 仿 真 计 算 (点击运行)", use_container_width=True):
    ...
    for _ in range(loop_count):
        ...
        v0_m = 0.0 if "SiC" in device_type else max(0.0, v_pk_m - r_m * i_pk)
        ...
        if "闭环" in sim_mode:
            ...
            tj_current = tj_new
                # 展示面板
    st.success(f"✅ 联合仿真执行完毕！(模式: {device_type} - {mode})")
